...
import { BlogEntrySchema, BlogEntryUpdateSchema } from '../api/schemas.js'
...
export const updateBlogEntry = new ValidatedMethod({
  name: 'updateBlogEntry',
  validate: BlogEntryUpdateSchema.validator(),
  run ({id, title, text, published}) {
    var author
    var now = new Date()
    ...
    BlogEntries.update({_id: id}, {$set: {author: author, title: title, text: text, published: published, updateDate: now}})
  }
})
